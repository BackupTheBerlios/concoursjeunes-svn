...
if(dbVersion == 0) {
	...
	if(!rowSet.first()) {
		...
		sql.executeUpdate("INSERT INTO BLASONS (NOMBLASON, HORIZONTAL_RATIO, VERTICAL_RATIO, NUMORDRE) "
			+ "SELECT DISTINCT CONCAT(BLASONS, 'cm') AS NOMBLASON, 1, 1, BLASONS FROM DISTANCESBLASONS WHERE BLASONS > 60 ORDER BY BLASONS DESC");
		sql.executeUpdate("INSERT INTO BLASONS (NOMBLASON, HORIZONTAL_RATIO, VERTICAL_RATIO, NUMORDRE) "
			+ "SELECT DISTINCT CONCAT(BLASONS, 'cm') AS NOMBLASON, 0.5, 1, BLASONS FROM DISTANCESBLASONS WHERE BLASONS > 40 BLASONS <= 60 ORDER BY BLASONS DESC");
		sql.executeUpdate("INSERT INTO BLASONS (NOMBLASON, HORIZONTAL_RATIO, VERTICAL_RATIO, NUMORDRE) "
			+ "SELECT DISTINCT CONCAT(BLASONS, 'cm') AS NOMBLASON, 0.5, 0.5, BLASONS FROM DISTANCESBLASONS WHERE BLASONS >= 40 ORDER BY BLASONS DESC");
		sql.executeUpdate("UPDATE DISTANCESBLASONS A SET NUMBLASON=(SELECT DISTINCT NUMBLASON FROM BLASONS B WHERE B.NOMBLASON = CONCAT(A.BLASONS, 'cm'));");
		
		sql.executeUpdate("INSERT INTO BLASONS (NOMBLASON, HORIZONTAL_RATIO, VERTICAL_RATIO, NUMORDRE) VALUES ('Tri Spot \"Vegas\"', 0.5, 0.5, 39);");
		sql.executeUpdate("INSERT INTO BLASONS (NOMBLASON, HORIZONTAL_RATIO, VERTICAL_RATIO, NUMORDRE) VALUES ('Tri Spot Vertical', 0.25, 1, 24);");
