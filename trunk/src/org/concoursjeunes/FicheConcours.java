...
public class FicheConcours {
    ...
    private ArcherList archerlist   = new ArcherList(parametre);
	 ...
	 */
	public ArcherList getArcherlist() {
		return archerlist;
	}
	 ...
	 * @uml.property  name="archerlist"
	 */
	public void setArcherlist(ArcherList archerlist) {
	...
	public boolean havePlaceForConcurrent(Concurrent concurrent) {
		...
		DistancesEtBlason db1 = DistancesEtBlason.getDistancesEtBlasonForConcurrent(reglement, concurrent);
		DistancesEtBlason db2 = DistancesEtBlason.getDistancesEtBlasonForConcurrent(reglement, conc2);
		...
		CriteriaSet cs1 = concurrent.getCriteriaSet();
		cs1.setFiltreCriteria(reglement.getPlacementFilter());
		CriteriaSet cs2 = conc2.getCriteriaSet();
		cs2.setFiltreCriteria(reglement.getPlacementFilter());
		
		//si il reste de la place dans la nouvelle categorie pas de pb
		place = getOccupationCibles(concurrent.getDepart()).get(cs1);
		if(place.getPlaceLibre() > 0 || getNbCiblesLibre(concurrent.getDepart()) > 0) {
			return true;
		}
		
		//si le retrait du concurrent libere une cible ok
		place = getOccupationCibles(concurrent.getDepart()).get(cs2);
    ...
    public boolean removeConcurrentInTeam(Concurrent removedConcurrent) {
        ...
        if(equipes != null) {
            remove = equipes.removeConcurrent(reglement, removedConcurrent);
        }
    ...
    public void placementConcurrents(int depart) {
        ...
        for(DistancesEtBlason distancesEtBlason : archerlist.listDistancesEtBlason(reglement, true, depart)) {
            //liste les archers pour le distance/blason
            Concurrent[] concurrents = ArcherList.sort(archerlist.list(reglement, distancesEtBlason, depart), ArcherList.SORT_BY_CLUBS);
    ...
    private Hashtable<DistancesEtBlason, OccupationCibles> calculatePlaceDisponible(int depart) {
        ...
        Hashtable<CriteriaSet, DistancesEtBlason> correspSCNA_DB = reglement.getCorrespondanceCriteriaSet_DB();
        ...
        for(DistancesEtBlason distblas : distancesEtBlasons) {
            ...
            nbParDistanceBlason[i] = archerlist.countArcher(reglement, distblas, depart);
    ...
    private void makePasDeTir() {
    	for(int i = 0; i < parametre.getNbDepart(); i++) {
    		...
    		for(int j = 0; j < parametre.getNbCible(); j++) {
    			Cible cible = new Cible(j+1, reglement, parametre.getNbTireur());
    ...
    private void classement(int depart) {
        ...
        for(Criterion criterion : reglement.getListCriteria()) {
            criteriaFilter.put(criterion, criterion.isClassement());
        }
        
        //Etablit le classement des concurrents en fonction du nombre de points obtenue.
        CriteriaSet[] catList = CriteriaSet.listeDifferentiationCriteria(reglement, criteriaFilter);
        ...
        for(int i = 0; i < catList.length; i++) {
            ...
            Concurrent[] sortList = ArcherList.sort(unsortList.toArray(new Concurrent[unsortList.size()]), ArcherList.SORT_BY_POINTS);
    ...
    public String getClassement(int outType, int depart) {
        ...
        if(archerlist != null && archerlist.countArcher() > 0) {
            ...
            CriteriaSet.sortCriteriaSet(scnaUse, reglement.getListCriteria());
            ...
            for(CriteriaSet scna : scnaUse) {
                ...
                if(sortList.length > 0) {
                        ...
                        double tailleChampDistance = 10.5262 / reglement.getNbSerie();
                    ...
                    tplClassement.parse("categories.NB_TIREUR_COLS", "" + (4 + reglement.getNbSerie())); //$NON-NLS-1$ //$NON-NLS-2$
                    tplClassement.parse("categories.NB_TIREURS", "" + sortList.length); //$NON-NLS-1$ //$NON-NLS-2$
                    
                    for(int j = 0; j < reglement.getNbSerie(); j++) {
                        scna.setFiltreCriteria(reglement.getPlacementFilter());
                        
                            tplClassement.parse("categories.distances.DISTANCE", //$NON-NLS-1$
                            		reglement.getCorrespondanceCriteriaSet_DB(scna).getDistance()[j] + "m"); //$NON-NLS-1$
                    ...
                    for(int j = 0; j < sortList.length; j++) {
                        ...
                        for(int k = 0; k < reglement.getNbSerie(); k++) {
    ...
    private String getXMLListeArcher(int mode, int depart) {
        ...
        for(Concurrent concurrent : ArcherList.sort(archerlist.list(depart), ArcherList.SORT_BY_NAME)) {
    ...
    private String getXMLEtiquettes(int nblarg, int nbhaut, int depart) {
        ...
        int colonne = 0;
        int ligne = 0;
        for(Concurrent concurrent : ArcherList.sort(archerlist.list(depart), ArcherList.SORT_BY_CIBLES)) {
            if(colonne == 0)
