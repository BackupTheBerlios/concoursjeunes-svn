...
public class EquipeDialog extends JDialog implements ActionListener, ListSelectionListener, TreeSelectionListener, MouseListener, MouseMotionListener {
	...
	CriteriaSet[] catList = CriteriaSet.listeDifferentiationCriteria(ficheConcours.getReglement(), criteriaFilter);
     ...
     * @param typeSort
     */
    private void listeConcurrents(CriteriaSet scna, int typeSort) {
     ...
     * @param typeSort
     */
    private void listeConcurrents(Entite forclub, CriteriaSet scna, int typeSort) {
	Concurrent[] unsortList;
	Entite[] compagnies;

	//determine l'étendu de la séléction
	if(forclub == null || forclub.equals("")) { //$NON-NLS-1$
	...
	for(Entite compagnie : compagnies) {
	    ...
	    Concurrent[] concurrents = ficheConcours.getArcherlist().list(compagnie, scna);
	    //regarde si leurs nombre est suffisant pour créer une équipe
	    if(!cbEquipeClub.isSelected() || concurrents.length >= ficheConcours.getReglement().getNbMembresRetenu()) {
	...
	for(Hashtable<String, Equipe> htable : hEquipes.values()) {
		...
		if(equipe.getMembresEquipe().size() < ficheConcours.getReglement().getNbMembresRetenu()) {
	...
	CriteriaSet scna = ((CriteriaSetLibelle)equipesTreeNode.getUserObject()).getDifferentiationCriteria();
	...
	if(hEquipes.get(scna).get(nomEquipe).getMembresEquipe().size() 
		+ selectionConc.length > ficheConcours.getReglement().getNbMembresEquipe()) {
	    JOptionPane.showMessageDialog(this,
	...
	while(edmtnRoot.hasMoreElements()) {
	    ...
	    if(((CriteriaSetLibelle)tnStmp.getUserObject()).getDifferentiationCriteria() == scna) {
		tnSCNA = tnStmp;
	...
	if(hEquipes.get(scna).get(equipe.getNomEquipe()).getMembresEquipe().size() < 
			ficheConcours.getReglement().getNbMembresRetenu()) {
	...
	if(e.getSource() instanceof JCheckBox) {
		...
		if(dmtn != null) {
			...
			listeConcurrents(((CriteriaSetLibelle)dmtnObj).getDifferentiationCriteria(), ArcherList.SORT_BY_CLUBS);
		    }
		}
	    }
		...
		if(dmtnObj instanceof CriteriaSetLibelle) {
		    ...
		    CriteriaSet scna = ((CriteriaSetLibelle)tnSCNA.getUserObject()).getDifferentiationCriteria();
		...
		if(dmtn != null) {
		    Object dmtnObj = dmtn.getUserObject();
		    if(dmtnObj instanceof CriteriaSetLibelle) {
			listeConcurrents(((CriteriaSetLibelle)dmtnObj).getDifferentiationCriteria(), ArcherList.SORT_BY_NAME);
		    }
		}
		...
		Object dmtnObj = dmtn.getUserObject();
		if(dmtnObj instanceof CriteriaSetLibelle) {
		    listeConcurrents((Entite)jcbClubs.getSelectedItem(), ((CriteriaSetLibelle)dmtnObj).getDifferentiationCriteria(), ArcherList.SORT_BY_NAME);
		}
	    }
	    }
	}
	...
	if(e.getSource() == ajlConcurrents) {
	    ...
	    if(selection.length > ficheConcours.getReglement().getNbMembresEquipe()) {
		tmplist.removeSelectionInterval(selection[ficheConcours.getReglement().getNbMembresEquipe()],
	...
	if(e.getSource() == ajlConcurrents) {
		...
		if(selectedPath != null) {
			...
			addMembresToEquipe(((CriteriaSetLibelle)((DefaultMutableTreeNode)selectedNode.getParent()).getUserObject()).getDifferentiationCriteria(), 
