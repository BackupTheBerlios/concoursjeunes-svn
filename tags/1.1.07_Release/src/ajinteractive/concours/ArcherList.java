...
//private:
	...
	/**
	 * Extrait la liste des concurrents pour un distance/blason donnée
	 * 
	 * @param distancesEtBlason
	 * @return Concurrent[] - la liste des concurrents correspondant aux criteres de recherche
	 */
	public Concurrent[] list(DistancesEtBlason distancesEtBlason) {
		ArrayList<Concurrent> sel = new ArrayList<Concurrent>();
        
		for(int i=0; i < this.archList.size(); i++) {
            DifferentiationCriteria dci = archList.get(i).getDifferentiationCriteria();
            dci.setFiltreCriteria(ficheConcours.parametre.getPlacementFilter());
            DistancesEtBlason db = ficheConcours.parametre.getCorrespondanceDifferentiationCriteria_DB(dci);
	...
	/**
	 * Donne la liste des distance blason utilise
	 * 
	 * @return DistancesEtBlason
	 */
	public ArrayList<DistancesEtBlason> listDistancesEtBlason() {
		return listDistancesEtBlason(false);
	}
	
	/**
	 * Donne la liste des distance blason utilise trie par db
	 * 
     * @param sort - est ce que la liste doit être trié?
	 * @return DistancesEtBlason
	 */
	public ArrayList<DistancesEtBlason> listDistancesEtBlason(boolean sort) {
		ArrayList<DistancesEtBlason> alDB = new ArrayList<DistancesEtBlason>();
		
		for(Concurrent concurrent: archList) {
			boolean add = true;
			for(DistancesEtBlason dbtemp : alDB) {
                DifferentiationCriteria dci = concurrent.getDifferentiationCriteria();
                dci.setFiltreCriteria(ficheConcours.parametre.getPlacementFilter());
                DistancesEtBlason db = ficheConcours.parametre.getCorrespondanceDifferentiationCriteria_DB(dci);
				if(dbtemp.equals(db)) {
					add = false;
	 ...
	 * @return int
	 */
	public int countArcher(DistancesEtBlason distancesEtBlason) {
		return list(distancesEtBlason).length;
	}
