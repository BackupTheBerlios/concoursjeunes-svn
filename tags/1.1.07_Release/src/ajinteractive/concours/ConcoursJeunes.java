...
public class ConcoursJeunes extends JFrame implements ActionListener, WindowListener {
    ...
    public static final long serialVersionUID          = 10110070019l;
    
    /**
     * Chaines de version de ConcoursJeunes
     */
    public static final String NOM                     = "ConcoursJeunes";      //$NON-NLS-1$
    public static final String VERSION                 = "1.10.07 - 26/01/2007";//$NON-NLS-1$
...
//	private
	...
	public ConcoursJeunes() {
        ...
        ajrLibelle = new AjResourcesReader(RES_LIBELLE, configuration.getLangue());
    ...
    class LoaderThread extends Thread {
        ...
        private boolean resultarcLoader() {
            ...
            try {
            	if(System.getProperty("os.name").startsWith("Windows")) {
	                Process proc = Runtime.getRuntime().exec(System.getProperty("user.dir")
	                        + File.separator + ajrParametreAppli.getResourceString("resultarc.export.cmd"));
	                proc.waitFor();
            	}
                ...
                FileReader frficlub = new FileReader(configuration.getRessourcesPath() + File.separator + ajrParametreAppli.getResourceString("resultarc.ficlub.fichier"));
                ...
                PrintStream psficlub = new PrintStream(configuration.getRessourcesPath() + File.separator + ajrParametreAppli.getResourceString("resultarc.ficlub.fichier"));
                psficlub.print(sFiclub);
                psficlub.close();
                //FIN HACK
                
                //chargement de chaque ligne de FICLUB
                sourceReader = new BufferedReader(new FileReader(configuration.getRessourcesPath() + File.separator + ajrParametreAppli.getResourceString("resultarc.ficlub.fichier")));
                String line;
                for(int i=0;(line = sourceReader.readLine()) != null;i++) {
                    listeEntite.add(AJToolKit.tokenize(line, separateur));
                }
                ...
                //chargement de chaque ligne BASE
                sourceReader = new BufferedReader(new FileReader(configuration.getRessourcesPath() + File.separator + ajrParametreAppli.getResourceString("resultarc.fichier")));
                for(int i=0;(line = sourceReader.readLine()) != null;i++) {
                    listelicencie.add(AJToolKit.tokenize(line, separateur));
                }
