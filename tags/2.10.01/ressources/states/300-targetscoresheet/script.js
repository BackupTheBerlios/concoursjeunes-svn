...
function printState(ficheConcours, template, document, writer) {
	...
	with(contexte) {
		...
		try {
			...
			//var concurrents = ConcurrentList.sort(ficheConcours.getConcurrentList().list(depart), SortCriteria.SORT_BY_CIBLES);
			...
			for(var i = 0; i < targets.size(); i++) {
				...
				var nbPositions = targets.get(i).getNbMaxArchers();
				...
				for(var j = 0; j < nbPositions; j++) {
					...
					if(concurrent != null) {
						...
						templateXML.parse("scoresheet.positions.emplacement", new TargetPosition(concurrent.getCible(), concurrent.getPosition()).toString());
					} else {
